...
export function registerV36Capabilities(server: McpServer, client: CoolifyClient): void {
  const call = async (name: string, input: RecordValue = {}): Promise<unknown> => {
    ...
    return (await client.request(operation, input)).data;
  };
  ...
  register("coolify_bulk_update_application_env", "Update an environment variable across applications.", z.object({ app_uuids: z.array(z.string()), key: z.string(), value: z.string(), is_build_time: z.boolean().optional() }), { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false }, async ({ app_uuids, key, value, is_build_time }) => batch((app_uuids as string[]).map((uuid) => ({ uuid })), (app) => call("coolify_update_env_by_application_uuid", { uuid: app.uuid, body: { key, value, is_buildtime: is_build_time ?? false } })));
