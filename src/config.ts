...
export function loadConfig(env: Record<string, string | undefined> = process.env): CoolifyConfig {
  ...
  try {
    ...
    throw new ConfigError("COOLIFY_URL must be a valid HTTP(S) URL");
  }
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new ConfigError("COOLIFY_URL must use http:// or https://");
  }
  if (url.search || url.hash) throw new ConfigError("COOLIFY_URL must not include a query or fragment");
