...
if (cursorVariables.type !== "object" || JSON.stringify(cursorVariables.required) !== "[]" || !("COOLIFY_BASE_URL" in cursorVariableProperties) || !("COOLIFY_ACCESS_TOKEN" in cursorVariableProperties)) throw new Error("Cursor plugin must declare optional Coolify connection variables");
if (cursorMcpEnv.CURSOR_COOLIFY_BASE_URL !== "${COOLIFY_BASE_URL}" || cursorMcpEnv.CURSOR_COOLIFY_ACCESS_TOKEN !== "${COOLIFY_ACCESS_TOKEN}" || "COOLIFY_BASE_URL" in cursorMcpEnv || "COOLIFY_ACCESS_TOKEN" in cursorMcpEnv) throw new Error("Cursor MCP must not override inherited Coolify connection variables");
