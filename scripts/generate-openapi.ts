...
for (const [path, pathItem] of Object.entries(document.paths ?? {})) {
  for (const [method, value] of Object.entries(pathItem)) {
    ...
    if (bodyType) {
      ...
      for (const combination of invalidCombinations ?? []) bodyText += `.refine((body) => !(${Object.entries(combination.values).map(([key, value]) => `body[${quote(key)}] === ${JSON.stringify(value)}`).join(" && ")}), { message: ${quote(combination.message)} })`;
    ...
    if (requireAnyQuery?.length) inputSchema += `.refine((input) => ${requireAnyQuery.map((key) => `typeof input[${quote(key)}] === "string" && input[${quote(key)}].trim().length > 0`).join(" || ")}, { message: ${quote(`At least one of ${requireAnyQuery.join(", ")} is required`)} })`;
    ...
    const title = name.replace(/^coolify_/u, "").replaceAll("_", " ").replace(/\buuid\b/giu, "UUID").replace(/^./u, (letter) => letter.toUpperCase());
    const description = /^(list|get|create|update|delete)$/iu.test(summary) ? title : summary;
...
await Bun.write(`${outputDir}/operations.ts`, ["// Generated by scripts/generate-openapi.ts.", 'import { z } from "zod";', "", "export type GeneratedOperation = { name: string; method: string; path: string; title: string; description: string; inputSchema: z.ZodType; responseSchema: z.ZodType; responseKind: string; parameters: Array<{ location: string; name: string }>; annotations: { readOnlyHint: boolean; destructiveHint: boolean; idempotentHint: boolean; openWorldHint: boolean } };", "", "export const operations: GeneratedOperation[] = [", operations.join(",\n"), "];", ""].join("\n"));
