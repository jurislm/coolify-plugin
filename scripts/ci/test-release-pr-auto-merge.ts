...
function candidate(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    base: { ref: "main", sha: BASE_SHA, repo: { full_name: "jurislm/coolify-plugin" } },
    head: { ref: RELEASE_BRANCH, sha: HEAD_SHA, repo: { full_name: "jurislm/coolify-plugin" } },
    ...overrides,
  };
}
...
async function testFailureGuards(): Promise<void> {
  ...
  await expectRejects(
    () => runReleasePrAutoMerge({
      ...
      fetchImpl: fakeFetch([
        jsonResponse([candidate({ base: { ref: "main", sha: BASE_SHA, repo: { full_name: "jurislm/coolify-plugin" } } })]),
        jsonResponse(candidate({ base: { ref: "main", sha: BASE_SHA, repo: { full_name: "jurislm/coolify-plugin" } } })),
